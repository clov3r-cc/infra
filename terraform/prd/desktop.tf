...
resource "random_password" "vm_root_password__desktop" {
...
resource "random_password" "vm_user_password__desktop" {
  for_each = local.vm_settings__desktop
...
resource "terraform_data" "cloud_init_config__desktop" {
  for_each = local.vm_settings__desktop
  ...
  provisioner "file" {
    content = templatefile("cloud-init/${local.env}-dsk_cloud-init.yaml.tftpl", {
      CI_HOSTNAME               = "${local.env}-dsk-${format("%02d", tonumber(each.key))}",
      CI_ROOT_PASSWORD          = random_password.vm_root_password__desktop[each.key].result,
      CI_MACHINEUSER_NAME       = local.machine_user,
      CI_MACHINEUSER_PASSWORD   = random_password.vm_user_password__desktop[each.key].result,
      CI_MACHINEUSER_SSH_PUBKEY = base64decode(local.vm_ssh_public_key),
    })
    destination = "/tmp/${local.env}-dsk-${format("%02d", tonumber(each.key))}_cloud-init.yaml"
  }
  provisioner "remote-exec" {
    inline = [
      "echo '${var.pve_user_password}' | sudo -S mv /tmp/${local.env}-dsk-${format("%02d", tonumber(each.key))}_cloud-init.yaml /var/lib/vz/snippets/",
    ]
  }
}
...
resource "proxmox_vm_qemu" "desktop" {
  for_each   = local.vm_settings__desktop
  depends_on = [terraform_data.cloud_init_config__desktop]

  name               = "${local.env}-dsk-${format("%02d", tonumber(each.key))}"
  target_node        = each.value.host_name
  vmid               = each.value.vm_id
  description        = "Desktop VM. This VM is managed by Terraform."
  bios               = "seabios"
  start_at_node_boot = true
  agent              = 1
  clone              = local.vm_template__alma
  full_clone         = true
  tags               = "${local.env};terraform;desktop;qdevice"
  qemu_os            = "l26"

  startup_shutdown {
    # 待機系を先に落とす（例: 2台あるときは、#1 -> 2 + 1 - 1 = 優先度 2、#2 -> 2 + 1 - 2 = 優先度 1）
    order            = length(local.vm_settings__desktop) + 1 - tonumber(each.key)
  ...
  os_type  = "cloud-init"
  cicustom = "user=local:snippets/${local.env}-dsk-${format("%02d", tonumber(each.key))}_cloud-init.yaml"
...
resource "ansible_host" "desktop" {
  for_each = proxmox_vm_qemu.desktop

  name   = each.value.name
  groups = [ansible_group.internal.name, ansible_group.desktop.name, ansible_group.zabbix_server__qdevice.name]
  variables = {
    ansible_host    = each.value.ssh_host
    heartbeat_nw_ip = split("/", split("ip=", each.value.ipconfig1)[1])[0]
    host_index      = tonumber(each.key)
    vm_id           = local.vm_settings__desktop[each.key].vm_id
  }
}
