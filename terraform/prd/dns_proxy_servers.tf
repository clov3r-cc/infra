...
resource "random_password" "vm_root_password__dns_proxy_server" {
  for_each = local.vm_settings__dns_proxy_server
...
resource "random_password" "vm_user_password__dns_proxy_server" {
  for_each = local.vm_settings__dns_proxy_server
...
resource "terraform_data" "cloud_init_config__dns_proxy_server" {
  for_each = local.vm_settings__dns_proxy_server
  ...
  provisioner "file" {
    content = templatefile("cloud-init/${local.env}-dpx_cloud-init.yaml.tftpl", {
      CI_HOSTNAME               = "${local.env}-dpx-${format("%02d", tonumber(each.key))}",
      CI_ROOT_PASSWORD          = random_password.vm_root_password__dns_proxy_server[each.key].result,
      CI_MACHINEUSER_NAME       = local.machine_user,
      CI_MACHINEUSER_PASSWORD   = random_password.vm_user_password__dns_proxy_server[each.key].result,
      CI_MACHINEUSER_SSH_PUBKEY = base64decode(local.vm_ssh_public_key),
    })
    destination = "/tmp/${local.env}-dpx-${format("%02d", tonumber(each.key))}_cloud-init.yaml"
  }
  provisioner "remote-exec" {
    inline = [
      "echo '${var.pve_user_password}' | sudo -S mv /tmp/${local.env}-dpx-${format("%02d", tonumber(each.key))}_cloud-init.yaml /var/lib/vz/snippets/",
    ]
  }
}
...
resource "proxmox_vm_qemu" "dns_proxy_server" {
  for_each   = local.vm_settings__dns_proxy_server
  depends_on = [terraform_data.cloud_init_config__dns_proxy_server]
  ...
  startup_shutdown {
    ...
    order            = length(local.vm_settings__dns_proxy_server) + 1 - tonumber(each.key)
  ...
  os_type  = "cloud-init"
  cicustom = "user=local:snippets/${local.env}-dpx-${format("%02d", tonumber(each.key))}_cloud-init.yaml"
...
resource "ansible_group" "proxy_server" {
  name = "proxy_server"
}

resource "ansible_host" "dns_proxy_server" {
  for_each = proxmox_vm_qemu.dns_proxy_server

  name   = each.value.name
  groups = [ansible_group.dmz.name, ansible_group.dns_server.name, ansible_group.proxy_server.name]
  variables = {
