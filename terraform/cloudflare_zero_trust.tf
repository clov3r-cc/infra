...
resource "cloudflare_zero_trust_tunnel_cloudflared" "tunnel__cloudflared-01" {
...
resource "cloudflare_record" "pxmx01-mng" {
  ...
  name    = "pxmx01-mng"
  content = cloudflare_zero_trust_tunnel_cloudflared.tunnel__cloudflared-01.cname
  type    = "CNAME"
  proxied = true
}

# Creates the configuration for the tunnel.
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "tunnel__cloudflared-01" {
  tunnel_id  = cloudflare_zero_trust_tunnel_cloudflared.tunnel__cloudflared-01.id
  account_id = local.cloudflare_account_id
  config {
    ingress_rule {
